...
static void render_frame(void) {
    ...
    /* Cursor-home, draw the new frame, *then* clear anything left over
     * below it - not clear-then-draw. Clearing first shows a blank
     * screen for one frame before the redraw lands, which is exactly
     * what reads as flicker (worse on Windows consoles than on most
     * Unix terminal emulators, which tend to coalesce the two draws).
     * Same total ANSI work, just reordered so the screen is never
     * actually blank. */
    fputs("\033[H", stdout);
