...
/* Real 1.TPMOS's own pieces/keyboard/plugins/keyboard_input.c AND
 * pieces/joystick/plugins/joystick_input.c both check this same shape
 * of lock file (gl_os_has_focus()) before writing to the shared
 * history file, rather than trusting OS window-manager focus routing -
 * confirmed the real reason: a joystick reading a raw device node has
 * no window-focus concept at all, so an explicit cooperative lock is
 * the actual mechanism, not a redundant check. system/gl_mirror.c
 * writes pieces/system/gl_focus.lock on startup (removed on exit) -
 * while it exists, gl_mirror is the authoritative live input source
 * (it forwards keys itself, see its own keyboard()/special_keyboard()),
 * and this process backs off so the same keypress can't double-fire
 * into pieces/apps/player_app/history.txt from both processes at once. */
static int gl_has_focus(void) {
    char path[PATH_BUF];
    struct stat st;
    snprintf(path, sizeof(path), "%s/pieces/system/gl_focus.lock", project_root);
    return stat(path, &st) == 0;
}

static void append_key(int key) {
    if (gl_has_focus()) return;
    char path[PATH_BUF];
