/* dump_rgb_png - DEBUG TOOL, not part of the game loop, never wired
 * into default_op.txt/pal/main_loop.pal. Reads whatever
 * ops/compose_rgb_frame.c most recently wrote
 * (pieces/display/rgb_frame.raw + its rgb_frame.receipt.txt for the
 * real dimensions - not a hardcoded duplicate of FRAME_W/FRAME_H,
 * so this can't silently drift out of sync with that file) and writes
 * a real PNG via stb_image_write (libraries/stb_image_write.h, public
 * domain, vendored from real 1.TPMOS rather than fetched - the exact
 * same header several 1.TPMOS pieces already use, e.g.
 * pieces/system/emoji_extract/).
 *
 * Exists because this agent has no way to look at system/gl_mirror's
 * actual GLUT window - the checksum/receipt pattern proves the BYTES
 * are correct, but a human (or an image-capable read of this PNG) is
 * still the only way to confirm the pixels look right - direct user
 * instruction: "u complained u couldn't view the current gl... use stb
 * image write... get it to create u a jpg/png u can view whenever u
 * need." Run manually whenever that's needed, e.g.:
